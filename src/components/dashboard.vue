	...
	export default{
			...
			SDK.call('devless','profile',[], function(res){
				...
				if(res.payload.error){
					that.$swal('Failed Login','Sorry! You are not authenticated','error');
					that.$router.push({ path: "/login"});
				}
				if(res.payload.result.status == 0){
					that.$swal('Pending Activation','Sorry, Your account has not been activated yet','details');
				}
				else{
					that.$swal("QuickMed Dashboard","Welcome! "+ res.payload.result.username,"success");
		...
		methods:{
			...
			updateDetails:function(){
				var that = this;
				SDK.addData("Ninja", "details", that.details, 
					function(res){
						/*console.log(res);*/
			...
			updateuserInfo: function(){
				var that = this;
				SDK.call('devless', 'updateProfile', [that.userInfo.email, '', that.userInfo.username, '+2330540420521',that.userInfo.first_name, that.userInfo.last_name], 
					function(res){
			...
			updateprofileInfo: function(){
				...
				SDK.updateData("Ninja", "details", "id", that.profileInfo.id, data, function(){
